...
public class SimpleVLayoutLoadmore<T> {
    ...
    public DataListCallBack getFirstLoadCallback() {
        DataListCallBack<T> callBack = new DataListCallBack<T>() {
            ...
            public void onSuccess(ListData<T> data) {
                if (isAutoSetEmptyView && data.getContent().size() == 0) {
                    if (mVListener != null) {
                ...
                mMLoadMoreAdapter.setData(data.getContent());
    ...
    public DataListCallBack getLoadMoreCallback() {
        DataListCallBack<T> callBack = new DataListCallBack<T>() {
            ...
            public void onSuccess(ListData<T> data) {
                ...
                mMLoadMoreAdapter.addData(data.getContent());
    ...
    public Observer<ListData<T>> getFirstLoadObserver() {
        return new Observer<ListData<T>>() {
            ...
            @Override
            public void onNext(ListData<T> data) {
                if (isAutoSetEmptyView && data.getContent().size() == 0) {
                    if (mVListener != null) {
                        mVListener.showEmptyView();
                    }
            ...
            public void onError(Throwable e) {
                ...
                mMLoadMoreAdapter.setData(null);
                if (loadCallback != null) {
                    loadCallback.onError(-1, e.getMessage());
                }
                if (mVListener != null) {
                    mVListener.showEmptyView();
                }
